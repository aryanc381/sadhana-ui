...
export function DailyView() {
  ...
  return <SidebarProvider><AppSidebar /><SidebarInset className="min-w-0 overflow-x-hidden"><header className="flex h-16 items-center border-b px-6"><div className="flex items-center gap-5"><SidebarTrigger /><Breadcrumb><BreadcrumbList><BreadcrumbItem><BreadcrumbLink render={<Link href="/dashboard" />}>Dashboard</BreadcrumbLink></BreadcrumbItem><BreadcrumbSeparator /><BreadcrumbItem><BreadcrumbPage>Daily View</BreadcrumbPage></BreadcrumbItem></BreadcrumbList></Breadcrumb></div></header><main className="min-w-0 flex-1 p-4">{loading ? <Skeleton className="h-[40rem] w-full" /> : !view ? <p className="text-sm text-muted-foreground">No daily view available.</p> : <div className="grid min-w-0 gap-[calc(0.75rem+0.15vw)] lg:grid-cols-[1fr_2fr_1fr]"><div className="space-y-[calc(0.75rem+0.15vw)]"><PreviousDayReview review={view.previous_day} /><TodayReview ticketId={view.today.ticket.id} review={view.today.review} /></div><TodayTasks ticket={view.today.ticket} skills={skills} onChange={updateTicket} /><div className="space-y-[calc(0.75rem+0.15vw)]"><Card className="h-40"><CardHeader><CardTitle>Calendar</CardTitle></CardHeader><CardContent className="text-sm text-muted-foreground">Calendar integration coming next.</CardContent></Card><Card className="min-h-80"><CardHeader><CardTitle>Calendar slots</CardTitle></CardHeader></Card></div></div>}</main></SidebarInset></SidebarProvider>
}
