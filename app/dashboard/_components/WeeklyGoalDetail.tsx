...
function TaskTable({ ticket, skills, skillName, onTicketChange }: { ticket: DailyTicket; skills: Skill[]; skillName: (skillId: string) => string; onTicketChange: (ticket: DailyTicket) => void }) {
  ...
  return <div className="overflow-x-auto rounded-md border"><Table><TableHeader><TableRow><TableHead>Task</TableHead><TableHead>Description</TableHead><TableHead>Status</TableHead><TableHead>Skill</TableHead><TableHead className="w-10" /></TableRow></TableHeader><TableBody>{ticket.tasks.map((task) => <TableRow key={task.id}><TableCell className="font-medium">{task.task_name}</TableCell><TableCell className="max-w-48 truncate">{task.task_description || "—"}</TableCell><TableCell><Select value={task.status} onValueChange={(value) => value && changeStatus(task.id, value as TaskStatus)}><SelectTrigger className="w-28"><Badge className={`pointer-events-none ${statusColors[task.status]}`}>{task.status}</Badge></SelectTrigger><SelectContent side="bottom" sideOffset={8} align="start" alignItemWithTrigger={false}>{taskStatuses.map((status) => <SelectItem className="py-2 hover:bg-accent hover:py-3" key={status} value={status}><Badge className={`pointer-events-none ${statusColors[status]} hover:bg-inherit hover:text-inherit`}>{status}</Badge></SelectItem>)}</SelectContent></Select></TableCell><TableCell><Select value={task.skill_id} onValueChange={(value) => value && changeSkill(task.id, value)}><SelectTrigger className="w-32"><Badge className="pointer-events-none hover:bg-inherit hover:text-inherit">{skillName(task.skill_id)}</Badge></SelectTrigger><SelectContent side="bottom" sideOffset={8} align="start" alignItemWithTrigger={false}>{skills.map((skill) => <SelectItem className="py-2 hover:bg-accent hover:py-3" key={skill.id} value={skill.id}><Badge className="pointer-events-none hover:bg-inherit hover:text-inherit">{skill.name}</Badge></SelectItem>)}</SelectContent></Select></TableCell><TableCell><DropdownMenu><DropdownMenuTrigger render={<Button variant="ghost" size="icon" className="cursor-pointer" />}><MoreHorizontal /></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem variant="destructive" onClick={() => removeTask(task.id)}>Delete</DropdownMenuItem></DropdownMenuContent></DropdownMenu></TableCell></TableRow>)}</TableBody></Table></div>
}
